...
//exchange these 2 imports for difference in bundle strategy
//import bpack from './packflat'
import bpack from './browser-pack'

import bresolve from './browser-resolve'
import cachedPathRelative from './cached-path-relative'
import depsSort from './deps-sort'
import insertGlobals from './insert-module-globals'
import mdeps from './module-deps'
import resolve from './resolve'
import { concat, readonly, splicer, through } from './streams'
import {createSyntaxError} from './syntax-error'
...
Browserify.prototype._syntax = function () {
    ...
    return through.obj(function (row, enc, next) {
        ...
        if (typeof self._syntaxCache[h] === 'undefined') {
            var err = createSyntaxError(row.source, row.file || row.id);
