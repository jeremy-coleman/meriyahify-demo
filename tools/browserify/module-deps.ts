...
import { combine, concat, duplexer, through } from "./streams";
...
ModuleDeps.prototype.resolve = function (id, parent, cb) {
  ...
  if (
    xhas(self.cache, parent.id, "deps", id) &&
  ...
  self.resolver(id, parent, function onresolve(err, file, pkg, fakePath) {
    ...
    if (!file)
      return cb(
        new Error('module not found: "' + id + '" from file ' + parent.filename)
      );
    ...
    if (!pkg || !pkg.__dirname) {
      self.lookupPackage(file, function (err, p) {
        ...
        onresolve(
          err,
          file,
          opts.packageFilter ? opts.packageFilter(p, p.__dirname) : p,
          fakePath
        );
      });
...
ModuleDeps.prototype.getTransforms = function (file, pkg, opts) {
  ...
  function makeTransform(tr, cb) {
    ...
    trOpts._flags = trOpts.hasOwnProperty("_flags")
      ? trOpts._flags
      : self.options;
...
ModuleDeps.prototype.walk = function (id, parent, cb) {
  ...
  self.resolve(id, parent, function (err, file, pkg, fakePath) {
    ...
    self.persistentCache(file, id, pkg, persistentCacheFallback, function (
      err,
      c
    ) {
    ...
    function persistentCacheFallback(dataAsString, cb) {
      var stream = dataAsString
        ? toStream(dataAsString)
        : self.readFile(file, id, pkg).on("error", cb);
      stream
        .pipe(
...
// if (
//   Array.isArray(this.options.noParse) &&
...
ModuleDeps.prototype.lookupPackage = function (file, cb) {
  ...
  (function next() {
    ...
    fs.readFile(pkgfile, function (err, src) {
      if (err) return onpkg();
      //tested & parses package.json correctly(although quotes are inconsistent - none or '')
      try {
        //@ts-ignore 
        ...
        return onpkg(
          new Error([err + " while parsing json file " + pkgfile].join(""))
        );
      }
      pkg.__dirname = dir;
